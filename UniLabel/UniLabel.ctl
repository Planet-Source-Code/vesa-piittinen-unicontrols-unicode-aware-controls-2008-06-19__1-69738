...
'*************************************************************************************************
'* UniLabel 2.9.9.1 - Unicode label user control
'* ---------------------------------------------
...
'* NOTES
'* -----
'* This control is not perfectly design time safe. It subclasses the container window, which
'* means there is a possibility of a crash. This crash most often occurs when code is edited
'* and the code refers to UniLabel. For example, removal of Form_Resize event with a simple
'* "UniLabel1.Move 0, 0, ScaleWidth" will cause the IDE to crash. Avoiding this issue is
'* easy enough: just close the visible form before removing a block of code.
'*
'* There is a property DesignTimeSafe. Set this to True to prevent crashes.
'*
'* HOW TO ADD TO YOUR PROGRAM
'* --------------------------
'* 1) Copy UniLabel.ctl and UniLabel.ctx to your project folder.
'* 2) In your project, add UniLabel.ctl.
'*
'* VERSION HISTORY
'* ---------------
'* 2.9.9.1 Release Candidate 1 (2008-06-17)
'* - Fix: Caption UTF-8 + PropertyBag issues (problematic use of StrConv)
'* - Generic improvements: Font changes are detected better, FontStrike -> FontStrikethru, other minor changes
'*
'* 2.5 (2007-12-15)
'* - Minor code tweaks, removed Private_InIDE
'*
'* 2.3 (2007-12-05)
'* - Added several font properties to workaround issues with font not changing when setting object properties
'* - BackStyle is restored!
'*
'* 2.2 (2007-11-19)
'* - Selfsub update: finally no more crashes with multiple controls! Thanks LaVolpe ^_^
'*   (I had a complex workaround planned but since Selfsub is now fixed... aah!)
'*
'* 2.1 (2006-09-03)
'* - Rewrite to fix issues with old subclassing code. UserControl is no more "InvisibleAtRuntime".
'* - Paints to the UserControl instead (for now).
'* - New events: Change, Click, DblClick, MouseDown, MouseEnter, MouseLeave, MouseMove, MouseUp.
'* - New properties: MouseIcon, MousePointer, Padding, RightToLeft, UseEvents, UseMnemonic.
'* - New methods: MouseOver, SetColor, SetPadding.
'* - Bug: BackStyle transparent does not work.
'*
'* 2.0 (2006-08-29)
'* - Initial release. Draws directly to the container.
'* - No events.
'* - Properties: Alignment, AutoSize, BackColor, BackStyle, Caption, Font, ForeColor, WordWrap.
'* - Bug: crashing issues; problems with SelfSub.
'*
'* CREDITS
'* -------
...
' BackStyle property
Public Enum UniLabelBackStyle
    [Transparent Background]
    [Opaque Background]
End Enum

' Mouse button constants
Public Enum UniLabelMouseButtonConstants
    [No Button] = 0
...
' Shift constants
Public Enum UniLabelShiftConstants
    [No Mask] = 0
...
Public Event Click(Button As UniLabelMouseButtonConstants)
Public Event DblClick(Button As UniLabelMouseButtonConstants)
Public Event MouseDown(Button As UniLabelMouseButtonConstants, Shift As UniLabelShiftConstants, X As Single, Y As Single)
Attribute MouseDown.VB_Description = "Occurs when the user presses the mouse button while an object has the focus."
Public Event MouseEnter()
Attribute MouseEnter.VB_Description = "Occurs when the mouse pointer enters the visible object area."
Public Event MouseLeave()
Attribute MouseLeave.VB_Description = "Occurs when the mouse pointer leaves the visible object area."
Public Event MouseMove(Button As UniLabelMouseButtonConstants, Shift As UniLabelShiftConstants, X As Single, Y As Single)
Attribute MouseMove.VB_Description = "Occurs when the user moves the mouse."
Public Event MouseUp(Button As UniLabelMouseButtonConstants, Shift As UniLabelShiftConstants, X As Single, Y As Single)
...
Dim m_BackStyle As UniLabelBackStyle    ' BackStyle
Dim m_Caption As String                 ' Caption
Dim m_DesignTimeSafe As Boolean         ' DesignTimeSafe
Dim WithEvents m_Font As StdFont        ' Font
...
Public Property Let BackColor(ByVal NewValue As OLE_COLOR)
    ...
    If m_BackStyle = [Opaque Background] Then Private_Redraw 0&
End Property
Public Property Get BackStyle() As UniLabelBackStyle
...
End Property
Public Property Let BackStyle(ByVal NewValue As UniLabelBackStyle)
    m_BackStyle = NewValue
    ' redraw
    If m_BackStyle = [Transparent Background] Then GetClientRect UserControl.ContainerHwnd, m_Container
    UserControl.BackStyle = m_BackStyle
End Property
Public Property Get Caption() As String
Attribute Caption.VB_Description = "Returns/sets the text displayed. Must be UTF-8 during design time!"
Attribute Caption.VB_ProcData.VB_Invoke_Property = ";Appearance"
Attribute Caption.VB_UserMemId = -518
Attribute Caption.VB_MemberFlags = "200"
    Dim bytCaption() As Byte, lngA As Long
    If Not blnDesignTime Then
        Caption = m_Caption
    Else
        bytCaption = Private_UTF16toUTF8(m_Caption)
        ReDim Preserve bytCaption(UBound(bytCaption) * 2 + 1)
        For lngA = UBound(bytCaption) - 1 To 2 Step -2
            bytCaption(lngA) = bytCaption(lngA \ 2)
            bytCaption(lngA + 1) = 0
        Next lngA
        bytCaption(1) = 0
        Caption = bytCaption
...
Public Property Let Caption(ByRef NewValue As String)
    ...
    ElseIf LenB(NewValue) Then
        bytCaption = NewValue
        For lngA = 2 To UBound(bytCaption) Step 2
            bytCaption(lngA \ 2) = bytCaption(lngA)
        Next lngA
        ReDim Preserve bytCaption((lngA \ 2) - 1)
        m_Caption = Private_UTF8toUTF16(CStr(bytCaption))
    Else
    ...
    If m_AutoSize Then Private_SetSize Else Private_Redraw
    ' if using events...
    If m_UseEvents And (Not blnDesignTime) Then RaiseEvent Change
End Property
Public Property Get DesignTimeSafe() As Boolean
Attribute DesignTimeSafe.VB_Description = "Returns/sets whether the object is perfectly safe in design time. If True, the object may not be perfectly responsive."
    DesignTimeSafe = m_DesignTimeSafe
End Property
Public Property Let DesignTimeSafe(ByVal NewValue As Boolean)
    If blnDesignTime Then
        m_DesignTimeSafe = NewValue
        PropertyChanged "DesignTimeSafe"
        MsgBox "Please close the window " & UserControl.Parent.Name
    End If
End Property
...
Attribute MouseOver.VB_Description = "Returns whether the mouse pointer is in the visible object area."
...
Attribute PaddingBottom.VB_Description = "Returns/sets the padding below the text."
...
Attribute PaddingLeft.VB_Description = "Returns/sets the padding left to the text."
...
Attribute PaddingRight.VB_Description = "Returns/sets the padding right to the text."
...
Attribute PaddingTop.VB_Description = "Returns/sets the padding above the text."
...
Attribute SetColor.VB_Description = "Sets colors of the object."
...
Attribute SetPadding.VB_Description = "Sets padding of the object."
    m_PaddingLeft = Left
    m_PaddingTop = Top
...
Attribute UseEvents.VB_Description = "Returns/sets whether the object triggers events."
...
Private Sub UserControl_InitProperties()
    ' design time?
    blnDesignTime = (Not UserControl.Ambient.UserMode)
    ' container scalemode
    ...
    m_AutoSize = False
    m_BackStyle = [Opaque Background]
    m_Caption = UserControl.Ambient.DisplayName
    m_DesignTimeSafe = False
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    m_BackStyle = PropBag.ReadProperty("BackStyle", [Opaque Background])
    UserControl.BackStyle = m_BackStyle
    bytCaption = PropBag.ReadProperty("Caption", UserControl.Ambient.DisplayName)
    m_Caption = bytCaption
    If blnDesignTime Then
        m_DesignTimeSafe = PropBag.ReadProperty("DesignTimeSafe", False)
    Else
        m_DesignTimeSafe = True
    End If
...
Private Sub UserControl_Resize()
    If m_RC.Bottom <> UserControl.ScaleHeight Or m_RC.Right <> UserControl.ScaleWidth Then
        If m_ContainerhWnd Then InvalidateRect m_ContainerhWnd, m_ContainerRC, -1&
        ' resize draw rectangle
        m_RC.Bottom = UserControl.ScaleHeight - m_PaddingBottom
        m_RC.Right = UserControl.ScaleWidth - m_PaddingRight
        ' see if autosize...
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    PropBag.WriteProperty "Caption", bytCaption
    PropBag.WriteProperty "DesignTimeSafe", m_DesignTimeSafe
...
Private Function Private_IsWinNT() As Boolean
    ...
    End If
End Function
Private Sub Private_Mnemonic()
    Dim lngPos As Long, strAccessKey As String
    ' reset
    AccessKeys = vbNullString
    ' see if there can be an access key
    If (m_UseMnemonic) And (LenB(m_Caption) > 2) Then
        Do
            ' find the ampersand
            lngPos = InStr(lngPos + 1, m_Caption, "&")
            ' if found and not the last character...
            If lngPos >= 1 And lngPos < Len(m_Caption) Then
                ' get the character
                strAccessKey = Mid$(m_Caption, lngPos + 1, 1)
                ' see if it is an ampersand
                If AscW(strAccessKey) = 38 Then
                    ' it is, skip it
                    lngPos = lngPos + 1
                Else
                    ' we got an access key!
                    AccessKeys = UCase$(strAccessKey)
                    ' done, quit loop
                    Exit Do
                End If
            Else
                ' no further ampersands are found, quit loop
                Exit Do
            End If
        Loop
    End If
...
Private Sub Private_Redraw(Optional EraseBG As Long = -1&)
    If m_BackStyle = [Opaque Background] Then
...
Private Sub Private_SetSize()
    Dim RC As RECT, lngTemp As Long, sngNewWidth As Single, sngNewHeight As Single, blnNeedRedraw As Boolean
    ...
    If m_AutoSize Then
        ...
        Else
            ...
            If (UserControl.Height <> sngNewHeight) Or (UserControl.Width <> sngNewWidth) Then
                ...
                If m_BackStyle = [Transparent Background] Then Private_Redraw
            End If
        End If
    Else
...
Private Sub Private_StartSubclass()
    ...
    If ssc_Subclass(UserControl.hWnd, , 1, , m_DesignTimeSafe, True) Then
        ' mouse events
...
Private Sub Private_WndProcContainer(ByVal bBefore As Boolean, ByRef bHandled As Boolean, ByRef lReturn As Long, ByVal lng_hWnd As Long, ByVal uMsg As Long, ByVal wParam As Long, ByVal lParam As Long, ByRef lParamUser As Long)
    ...
    If m_BackStyle = [Opaque Background] Then Exit Sub
    ' BEFORE OR AFTER?
    If bBefore Then
...
Private Sub Private_WndProcUserControl(ByVal bBefore As Boolean, ByRef bHandled As Boolean, ByRef lReturn As Long, ByVal lng_hWnd As Long, ByVal uMsg As Long, ByVal wParam As Long, ByVal lParam As Long, ByRef lParamUser As Long)
    ...
    Dim lnghDC As Long, lngBitmap As Long, lngBrush As Long, lngOldBrush As Long
    Dim rcDraw As RECT, rcRedraw As RECT, ptDraw As POINTAPI, lngTemp As Long, RC As RECT
    Dim XY(1) As Integer, Button As UniLabelMouseButtonConstants, Shift As UniLabelShiftConstants
    ' BEFORE or AFTER?
    If bBefore Then
        If uMsg = WM_PAINT Then
            ' we do not draw if we are in transparent mode
            If m_BackStyle = [Transparent Background] Then Exit Sub
        ...
        ElseIf uMsg = WM_DESTROY Then
            ssc_UnSubclass lng_hWnd 'UserControl.ContainerHwnd
        ElseIf m_UseEvents Then
            Select Case uMsg
                Case WM_LBUTTONDOWN, WM_MBUTTONDOWN, WM_RBUTTONDOWN
                    ...
                    ' see key states
                    If (wParam And MK_SHIFT) = MK_SHIFT Then Shift = [Shift Mask]
                    If (wParam And MK_CONTROL) = MK_CONTROL Then Shift = Shift Or [Ctrl Mask]
                    ' button states
        ...
        Select Case uMsg
            ...
            Case WM_SHOWWINDOW
                If wParam = 1& Then
                    ...
                    If ssc_Subclass(m_ContainerhWnd, , 2, , m_DesignTimeSafe, True) Then
                        ssc_AddMsg m_ContainerhWnd, MSG_AFTER, WM_EXITSIZEMOVE, WM_SHOWWINDOW
                        ...
                        On Error GoTo 0
                        Debug.Print UserControl.Ambient.DisplayName & ": Started subclassing! " & Hex$(m_ContainerhWnd)
                    Else
                        m_ContainerhWnd = 0
                        Debug.Print UserControl.Ambient.DisplayName & ": Container subclassing failed!"
                    End If
                ElseIf m_ContainerhWnd <> 0 Then
                    Debug.Print UserControl.Ambient.DisplayName & ": Ended subclassing! " & Hex$(m_ContainerhWnd)
                    ' unsubclass
                    If (Not m_ContainerIsParent) Or blnDesignTime Then ssc_UnSubclass m_ContainerhWnd
                    m_ContainerhWnd = 0
                Else
